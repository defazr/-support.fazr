...
### 프로젝트 상태: ✅ 운영 중 + UX 마이크로 개선 4개 + 모바일 가독성 + 코드 프리즈 (4/23~4/24)
...
- updates 글 **8개** (국민비서알림 + 지급수단가이드 + 속보-정부발표 + 속보-추경통과 + 지급안내 + 신청방법 + 인구감소지역 + 기존 1개)
- "정부24" 표현: **사이트 전체 제거** (면책 조항만 "정부 공식 사이트 아님")
- 날짜: 전부 확정형 (4월 27일, 5월 18일, 8월 31일)
- 건보료 컷오프: "5월 중 발표 예정" 유지 (미확정)
- FAQ: **20개 항목** (신규: "지금 바로 신청할 수 있나요?")
- 국민비서: 4채널 명시 (ips.go.kr / 네이버앱 / 카카오톡 / 토스) — 정책브리핑 출처 확인됨
...
- OG 이미지: 3페이지 모두 og-main.jpg (4/22 수정 — calculator/eligibility 누락 수정)
...
4/22 작업 효과 관측 기간. 코드 변경 금지.
- 이탈률 95% → 80~85% 목표
- 페이지/사용자 1.04 → 1.3~1.6 목표
- 작업1 블록 CTR 3~5% 기대 (1% 미만 시 위치 이동 발동)
...
1. **4/23~4/24**: 코드 프리즈 — 데이터 관측만
2. **4/25**: GPT 4/27 전환 설계 + 4/22 효과 분석 + Card 배경/블록 위치 판단
3. **4/26**: 4/27 통합 지시서 작성
4. **4/27**: 1차 지급 시작 대응 (스키니바 + Hero + status + 신규 글 + 요일제)
5. **5월 초**: 건보료 컷오프 발표 반영
6. calculator 취약계층 로직 보완 (비수도권·인구감소지역 +5만원)
7. 전담 콜센터 번호 확정 시 FAQ Q18 업데이트
8. GA4 + AdSense 수익 지표 비교
